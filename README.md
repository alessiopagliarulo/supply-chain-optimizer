...
Three results, each generated by a command in this repo and pinned to a committed JSON
artifact that CI diffs against the prose (`tests/test_docs_match_artifacts.py`).

- **Cost-vs-tail-risk frontier — 387 λ-solves** over 10 BOMs × 3 volumes × a 36-cell
  sensitivity grid (347 converged; the 40 that did not are excluded from every figure).
...
GET  /api/v1/feeds/status                    # feed freshness: download date AND observation date per feed
...
can merge to `main` (see `docs/archive/AUTONOMOUS-LOOP.md`). A running list of the loop's own
mistakes is kept alongside it because early
