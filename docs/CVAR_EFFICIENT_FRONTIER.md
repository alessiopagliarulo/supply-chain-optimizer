...
![Cost vs. CVaR-95 efficient frontier: expected cost on the x-axis, CVaR-95 on the y-axis, nine lambda-solves falling from the risk-neutral plan down to the knee at lambda = 0.3 and then flattening. A dashed chord marks the $4.27-per-$1 stretch.](cvar_frontier.png)

*Generated by `cd backend && python -m seeds.render_cvar_frontier_chart`, which reads
`cvar_frontier.json` — the same artifact every number in this document is generated from.
Expected cost runs along the x-axis, CVaR-95 — the **mean cost of the worst 5% of
scenarios** — along the y-axis, and each point is one λ-solve. The dashed chord is the
published headline made visible: $4.27 of tail risk removed per $1 of expected cost is the
AVERAGE slope of that chord from λ = 0 to the knee, not a marginal rate at any point on it.
Past the knee the same trade returns $0.41. Both figures are specific to this BOM at 60,000
units; at 100× and 1,000× volume the `knee` is `null` because the frontier is flat and there
is no trade-off to price.*

> **The live page and this chart can legitimately disagree, and here is why.**
> ![The deployed Frontier page](screenshots/current/11-frontier.png)
> The screenshot above is the deployed page. Its tiles read **$2.88 per $1**, not $4.27 —
> and the badge in its top-right says **5/7 λ solved**. The page re-solves on demand under a
> wall-clock budget on a free-tier instance, so it frequently serves a partial sweep and
> computes the knee from the λ values that came back. This document's $4.27 is the complete
> offline run. Neither number is wrong; they describe different solve sets, and the page says
> which one it is showing. Quote the artifact's figure, and quote the condition with it.
