...
- **Known limitations (carried over from the 2026-07-01 gap audit):**
  ...
  - 29 distributors share one Shenzhen city point (real city, but not each
    distributor's actual address) - a resolution shortcut, not fabricated data.
    See section 2.
...
- **Source:** each distributor's HQ or warehouse **city**, manually compiled
  from company websites, SEC filings, and public press releases (see the
  in-code comment above the dict). Precision is one point per city, not a
  street address: 91 of the 92 distributors sit on 43 distinct points, and
  29 distributors share one Shenzhen city point.
- **One distributor has no location:** VNN Services is in the snapshot as a
  supplier name with zero priced offers, and no address could be verified
  (its site publishes none). It is served with NULL coordinates, city and
  country (`seeds/seed_db.py::UNLOCATED_DISTRIBUTORS`, migration 0010).
  Until 2026-09-19 the seeder gave any unmapped distributor San Francisco's
  coordinates and the city "Unknown", so it would have been plotted as real.
- **Checked against OpenStreetMap (2026-09-19):** every coordinate must lie
  within 25 km of an independent OpenStreetMap Nominatim point for the city
  it is labelled with. The reference points are committed in
  `backend/seeds/data/distributor_city_centroids.json` (data (c)
  OpenStreetMap contributors, ODbL), regenerated by
  `python3 -m seeds.geocode_distributor_cities`, and enforced by
  `backend/tests/test_distributor_catalogue_api.py`.
- **Fixed by that check:** seven coordinates were 33-460 km from their own
  label and were moved to the labelled city's OpenStreetMap point: Schukat
  (plotted in Bavaria; the company is in Monheim am Rhein, North
  Rhine-Westphalia, per its Dusseldorf commercial register entry HRB 45473,
  so the label was corrected too), Rapid Electronics (Colchester), Conrad
  (Wernberg-Köblitz), Rochester Electronics (Newburyport), Jameco (Belmont),
  Avnet and Verical (Chandler). The city labels themselves, other than
  Schukat's, were not re-verified.
- **Served:** `GET /api/v1/distributors` (with `shares_location_with`, so a
  map can cluster stacked markers, and `located_only=true` to drop the
  unlocated row) and
  `GET /api/v1/catalogue/provenance`, which returns this precision,
  the tolerance, the live row counts and the "frozen 2024 snapshot, not a
  live feed" statement for any screen that shows the data.
